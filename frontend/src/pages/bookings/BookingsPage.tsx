...
export default function BookingsPage() {
  ...
  const [defaultRange, setDefaultRange] = useState<{ start: Date; end: Date } | null>(null)
  ...
  return (
    <Box>
      ...
      <Stack direction="row" spacing={2} sx={{ mb: 2, flexWrap: 'wrap' }}>
        <SearchField value={search} onChange={setSearch} placeholder="Search bookings..." />
        <TextField
          select
          size="small"
          label="Status"
      ...
      <Box sx={{ bgcolor: 'background.paper', borderRadius: 2, p: view === 'calendar' ? 2 : 0 }}>
        {view === 'calendar' ? (
          <FullCalendar
            ...
            <TableBody>
              {bookings.map((b) => (
                <TableRow key={b.id} hover>
                  ...
                  <TableCell>{format(new Date(b.end_time), 'h:mm a')}</TableCell>
                  <TableCell>
                    <StatusChip status={b.status} />
                  </TableCell>
                  <TableCell align="right">
                    {canManage(b) && b.status !== 'Cancelled' && b.status !== 'Completed' && (
                      <>
                        <IconButton
                          size="small"
                          onClick={() => {
